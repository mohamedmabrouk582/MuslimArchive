...
import com.mabrouk.quran.presentaion.utils.DOWNLOAD_VERSES_IDS
...
class SurahDownloadWorker @AssistedInject constructor(
  ...
  //  val event: EventBus
    ...
    override suspend fun doWork(): Result {
        ...
        inputData.getIntArray(VERSES_IDS)?.toList()?.apply {
            data.putIntArray(VERSES_IDS,this@apply.toIntArray())
        }?.forEach {
            repository.getSurahById(it)
                .firstOrNull()?.nameArabic?.let { it1 ->
                  //  event.sendType(it1)
                }
